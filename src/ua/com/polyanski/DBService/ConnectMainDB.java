...
public class ConnectMainDB extends ConnectDB {
        ...
        try {
            ...
            while(res.next()) {
                goods.add(new Good(res.getString("barcode"), res.getInt("id"), res.getString("name"),
                        res.getString("name_1"), res.getString("name_2"), res.getString("expiration_date"),
    ...
    public void update(Good good) {
            ...
            sql = "update Good\n" +
                    "    set type = (select id from spr_Type where name = '" + good.getType() + "'),\n" +
                    "    name = (select id from spr_Name where name = '" + good.getName() + "'),\n" +
                    "    model = (select id from spr_Model where name = '" + good.getModel() + "'),\n" +
